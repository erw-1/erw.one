...
const IDS = {
  ...
  // links
  hier : 'link_hier',  // hierarchy lines
  tag1 : 'link_tag1',  // 1 same‑tag cross‑links
  tag2 : 'link_tag2',  // 2 same‑tag cross‑links
  tag3 : 'link_tag3',  // 3+ same‑tag cross‑links
...
function buildGraphData () {
  ...
  // Tag‑overlap edges (shared‑tag cross‑links) ------------------------------
  const tagBuckets = new Map();
  pages.forEach(p => p.tagsSet.forEach(t => {
    (tagBuckets.get(t) ?? tagBuckets.set(t, []).get(t)).push(p);
  }));

  tagBuckets.forEach(list => {
    for (let i = 0; i < list.length; i++) {
      for (let j = i + 1; j < list.length; j++) {
        const a = list[i], b = list[j];
        if (adj.get(a._i)?.has(b._i)) continue;          // avoid dupes
...
function buildGraph (currentPage) {
  ...
  ['mini', 'full'].forEach(view => {
    const svg = KM.d3.select('#' + view).attr('class', 'km-graph');
    ...
    // Filter nodes/links for mini view (just immediate neighbourhood) -------
    let localNodes = nodes, localLinks = links;
    if (!full) {
      const focus = currentPage._i;
      const neighbourhood = new Set([focus, ...(adj.get(focus) || [])]);
      localNodes = nodes.filter(n => neighbourhood.has(n.id));
      localLinks = links.filter(l => neighbourhood.has(l.source.id ?? l.source) &&
                                     neighbourhood.has(l.target.id ?? l.target));
    }

    // D3 force‑directed simulation ------------------------------------------
    const sim = KM.d3.forceSimulation(localNodes)
      .force('link',  KM.d3.forceLink(localLinks).id(d => d.id).distance(50))
      .force('charge', KM.d3.forceManyBody().strength(-200))
      .force('center', KM.d3.forceCenter(w / 2, h / 2));

    // Extra home “pull” on the current node ---------------------------------
    sim.force('home', alpha => {
      const n = localNodes.find(n => n.ref === currentPage);
      if (!n) return;
      n.vx += (w / 2 - n.x) * 0.12 * alpha;
      n.vy += (h / 2 - n.y) * 0.12 * alpha;
    });
    ...
    // ---------------------------------------------------------------- Hover
    node.on('mouseover', (_, d) => fadeLinks(d.id, 0.1))
        .on('mouseout',  ()   => fadeLinks(null, 1));

    function fadeLinks (id, opacity) {
      link.transition().duration(200)
        .style('opacity', l =>
          id == null || l.source.id === id || l.target.id === id ? 1 : opacity);
    }
    ...
    // Re‑heat when zooming modal (full) -------------------------------------
    if (full) {
      new ResizeObserver(() => {
        const w = +svg.attr('width');
        const h = +svg.attr('height');
        sim.force('center', KM.d3.forceCenter(w / 2, h / 2)).alpha(0.5).restart();
      }).observe(svg.node());
    }
  });
}/* *********************************************************************
