...
/* ─────────────────────────────── Public API ────────────────────────────── */
/**
 * Global namespace for tiny helpers and lazy loaders exposed for debugging
 * and integration hooks. This object is intentionally small.
 */
window.KM = window.KM || {};
const KM = window.KM;
 ...
 * - Supports common conveniences: class / className, dataset, direct property assignment.
 * - Unknown keys are set via setAttribute.
 */
...
/**
 * Parse a “Markdown bundle” (sequence of `<!-- meta --> <content>` blocks).
 * Expected header meta in the comment: key:"value"
 */
function parseMarkdownBundle(txt) {
  ...
  // parent/children, tags, search strings
  pages.forEach((p) => {
    if (p !== root) {
  ...
  // Fence-aware sections (for deep-search)
  pages.forEach((p) => {
...
/**
 * Attach “secondary home” nodes directly under root to surface disconnected clusters.
 * Picks the largest representative per cluster.
 */
...
/** Compute stable hash path for a page (id chain from root) */
...
/** Resolve a sequence of ids into a page by walking the tree from root */
const find = (segs) => {
  let n = root;
...
/** Navigate to a page by updating the hash (kept public) */
function nav(page) {
...
/** Lazy-load minimal D3 surface needed for the mini-graph */
...
/** Lazy-load highlight.js and optionally register requested languages */
...
/** Sync HLJS theme with current UI theme */
...
/** Lazy-load marked and small plugins */
let mdReady = null;
KM.ensureMarkdown = () => {
...
/** Lazy-load KaTeX + auto-render and wire globals */
...
/** Copy text to clipboard with a tiny flash affordance */
...
/** Assign deterministic ids to headings (H1–H5) for deep-links */
...
/** Observe content headings and build a live ToC with “current” sync */
...
/** Prev / Next links among siblings */
...
/** “See also” section based on shared tags */
...
/** Fix footnote links to work with page base hashes */
...
/** Upgrade external links for safety; optionally open in new tab */
function hardenLinks() {
  const origin = location.origin.replace(/\/$/, '');
  $$('#content a[href^="http"]').forEach((a) => {
    try {
...
/** Small SVG button factory used in headings and code blocks */
...
/** Add “copy link” affordances to headings */
function decorateHeadings(page) {
  const base = hashOf(page);
  $$('#content h1,h2,h3,h4,h5').forEach((h) => {
    const url = `${location.origin}${location.pathname}#${base ? base + '#' : ''}${h.id}`;
    const btn =
      h.querySelector('button.heading-copy') ||
      h.appendChild(
        iconBtn('Copy direct link', ICONS.link, 'heading-copy', (e) => {
          e.stopPropagation();
...
/** Build the navigation tree (primary + grouped “secondary homes”) */
function buildTree() {
  const ul = $('#tree');
  if (!ul) return;
  ul.innerHTML = '';
...
/** Full-text search across titles/tags and section bodies */
function search(q) {
  const resUL = $('#results'),
    treeUL = $('#tree');
...
/** Render breadcrumb with sibling and children dropdowns */
...
function highlightCurrent(force = false) {
  ...
  const g = graphs.mini;
  g.node.attr('id', (d) => (d.id === id ? IDS.current : d.ref.children.length ? IDS.parent : IDS.leaf)).attr('r', (d) => (d.id === id ? 8 : 6));
...
/** Render a page into #content and wire all decorations */
async function render(page, anchor) {
  ...
  if (anchor) DOC.getElementById(anchor)?.scrollIntoView({ behavior: smooth, block: 'start' });
}

let currentPage = null;

/** Router: parse hash → render page and/or jump to anchor */
function route() {
  closePanels();
  const seg = location.hash.slice(1).split('#').filter(Boolean);
  const page = find(seg);
...
/** Close all off-canvas panels */
function closePanels() {
  $('#sidebar')?.classList.remove('open');
  $('#util')?.classList.remove('open');
}

/** Wire up UI shell: theme, sidebar, util panel, search, responsive bits */
function initUI() {
  ...
  (function themeInit() {
    ...
    function apply(isDark) {
      ...
      // Keep HLJS stylesheet synced with theme
      KM.ensureHLJSTheme();
    }
  })();
  ...
  // Build mini-graph once it scrolls into view
  ...
  // Fullscreen toggle for the mini-graph
  const mini = $('#mini');
  $('#expand').onclick = () => {
  ...
  // Responsive: close panels when expanding, and keep graph centered in FS
  addEventListener(
    'resize',
    () => {
  ...
  // Sidebar interactions (caret toggle / auto-close after link click)
  ...
  // Results interactions (auto-close)
  ...
  // Warm up HLJS in idle time (fast first highlight later)
  whenIdle(() => {
    KM.ensureHighlight();
  });
}
...
(async () => {
  try {
    ...
    // Let layout settle a frame before centering the graph
