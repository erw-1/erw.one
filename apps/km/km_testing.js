...
/** Normalize anchors inside a container. */
function normalizeAnchors(container = $('#content'), page, { onlyFootnotes = false } = {}) {
    const base = hashOf(page);
    if (!base) return;
    $$('a[href^="#"]', container).forEach(a => {
        ...
        // For general case: if the link does not resolve to a page, rewrite as page-local
        if (!parseTarget(href)) {
...
function initUI() {
    ...
    // Shared copy buttons (main content)
