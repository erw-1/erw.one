...
/** Get SVG#mini size (fullscreen-aware). */
...
/** Update mini graph viewport and re-center simulation. */
let _miniKick = 0;
...
function recenterNodes() {
  ...
  nodes.forEach(d => { d.x += tx; d.y += ty; });
}

/** Build graph nodes and links from page hierarchy and tags. */
...
/** Build the mini force-directed graph on first visibility. */
export async function buildGraph() {
  ...
  const label = view.append('g').selectAll('text')
    ...
    .attr('id', IDS.label).attr('font-size', 10).attr('pointer-events', 'none')
    .text(d => d.label);
...
/** Highlight the current page's node and center it. */
...
/** Keep mini-graph responsive to size/fullscreen changes. */
